...
export default function Home() {
  ...
  const handleCalculate = async (e: React.FormEvent) => {
    ...
    setLoading(true);
    try {
      // Points directly to your live production engine deployment
      ...
      console.error("Data pipeline processing exception:", error);
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <main style={{ backgroundColor: '#0b1329', color: '#f1f5f9', minHeight: '100vh', padding: '3rem 2rem', fontFamily: 'system-ui, -apple-system, sans-serif' }}>
      ...
      {/* Structural Animations Style Injection */}
      ...
      <div style={{ maxWidth: '1400px', margin: '0 auto', display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(450px, 1fr))', gap: '2.5rem' }}>
        ...
        <section className="animate-fade-in interactive-card" style={{ backgroundColor: '#111c44', padding: '2.5rem', borderRadius: '1.5rem', border: '1px solid #1e293b', boxShadow: '0 20px 25px -5px rgba(0, 0, 0, 0.3)' }}>
          <h2 style={{ color: '#34d399', fontSize: '1.6rem', fontWeight: '800', marginTop: 0, marginBottom: '2rem' }}>Diagnostics Configuration</h2>
          ...
          <form onSubmit={handleCalculate} style={{ display: 'flex', flexDirection: 'column', gap: '1.75rem' }}>
            ...
            <div style={{ borderLeft: '3px solid #3b82f6', paddingLeft: '1rem' }}>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1rem' }}>
                <div>
                  <label htmlFor="distance_km" style={{ display: 'block', fontSize: '0.85rem', color: '#94a3b8', fontWeight: '600', marginBottom: '0.5rem' }}>Monthly Logged Distance (km)</label>
                  <input
                    id="distance_km"
                    type="number"
                ...
                <div>
                  <label htmlFor="transport_type" style={{ display: 'block', fontSize: '0.85rem', color: '#94a3b8', fontWeight: '600', marginBottom: '0.5rem' }}>Transport Vector Profile</label>
                  <select
                    id="transport_type"
                    className="form-input-element"
            ...
            <div style={{ borderLeft: '3px solid #10b981', paddingLeft: '1rem' }}>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1rem' }}>
                <div>
                  <label htmlFor="electricity_kwh" style={{ display: 'block', fontSize: '0.85rem', color: '#94a3b8', fontWeight: '600', marginBottom: '0.5rem' }}>Grid Infrastructure Power (kWh)</label>
                  <input
                    id="electricity_kwh"
                    type="number"
                    className="form-input-element"
                ...
                <div>
                  <label htmlFor="grid_intensity_factor" style={{ display: 'block', fontSize: '0.85rem', color: '#94a3b8', fontWeight: '600', marginBottom: '0.5rem' }}>Regional Carbon Intensity Index</label>
                  <input
            ...
            <div style={{ borderLeft: '3px solid #f59e0b', paddingLeft: '1rem' }}>
              ...
              <div>
                <label htmlFor="diet_type" style={{ display: 'block', fontSize: '0.85rem', color: '#94a3b8', fontWeight: '600', marginBottom: '0.5rem' }}>Nutritional Consumption Protocol</label>
                <select
                  id="diet_type"
                  className="form-input-element"
            ...
            <div style={{ borderLeft: '3px solid #ec4899', paddingLeft: '1rem' }}>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1rem' }}>
                <div>
                  <label htmlFor="waste_kg" style={{ display: 'block', fontSize: '0.85rem', color: '#94a3b8', fontWeight: '600', marginBottom: '0.5rem' }}>Monthly Solid Waste Output (kg)</label>
                  <input
                    id="waste_kg"
                    type="number"
                    className="form-input-element"
                ...
                <div>
                  <label htmlFor="recycling_habits" style={{ display: 'block', fontSize: '0.85rem', color: '#94a3b8', fontWeight: '600', marginBottom: '0.5rem' }}>Recycling Diversion Strategy</label>
                  <select
                    id="recycling_habits"
                    className="form-input-element"
          ...
          {results && results.metrics ? (
            <>
              <div className="animate-fade-in results-card" style={{ background: 'radial-gradient(circle at top left, #1a234a, #111a3a)', padding: '2.5rem', borderRadius: '1.5rem', border: '1px solid #233166', textAlign: 'center', boxShadow: '0 20px 25px -5px rgba(0, 0, 0, 0.4)' }}>
                <h3 style={{ fontSize: '1.3rem', fontWeight: '600', color: '#94a3b8', margin: 0 }}>Computed Annual Projection Baseline</h3>
                ...
                <div style={{ backgroundColor: '#0b1329', padding: '0.75rem 1.25rem', borderRadius: '0.75rem', display: 'inline-block', border: '1px solid #1e293b' }}>
                  <span style={{ fontSize: '0.9rem', color: '#94a3b8' }}>Target Stabilization Variable: </span>
              ...
              <div className="animate-fade-in interactive-card" style={{ backgroundColor: '#111c44', padding: '2rem', borderRadius: '1.5rem', border: '1px solid #1e293b' }}>
                <h4 style={{ fontSize: '1.15rem', fontWeight: '700', color: '#f8fafc', marginTop: 0, marginBottom: '1.5rem' }}>Isolated Diagnostics Vector Output</h4>
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1.25rem' }}>
                  <div style={{ backgroundColor: '#0b1329', padding: '1rem 1.25rem', borderRadius: '1rem', border: '1px solid #1e293b' }}>
                    ...
                    <div style={{ fontSize: '1.5rem', fontWeight: '800', marginTop: '0.25rem' }}>{results.metrics.mobility_kg} <span style={{ fontSize: '0.85rem', color: '#64748b' }}>kg</span></div>
                  </div>
                  <div style={{ backgroundColor: '#0b1329', padding: '1rem 1.25rem', borderRadius: '1rem', border: '1px solid #1e293b' }}>
                    <div style={{ fontSize: '0.8rem', color: '#10b981', fontWeight: '700' }}>GRID ENERGY SECTOR</div>
                    <div style={{ fontSize: '1.5rem', fontWeight: '800', marginTop: '0.25rem' }}>{results.metrics.energy_kg} <span style={{ fontSize: '0.85rem', color: '#64748b' }}>kg</span></div>
                  </div>
                  <div style={{ backgroundColor: '#0b1329', padding: '1rem 1.25rem', borderRadius: '1rem', border: '1px solid #1e293b' }}>
                    <div style={{ fontSize: '0.8rem', color: '#f59e0b', fontWeight: '700' }}>NUTRITIONAL LIFE-STAGE</div>
                    <div style={{ fontSize: '1.5rem', fontWeight: '800', marginTop: '0.25rem' }}>{results.metrics.diet_kg} <span style={{ fontSize: '0.85rem', color: '#64748b' }}>kg</span></div>
                  </div>
                  <div style={{ backgroundColor: '#0b1329', padding: '1rem 1.25rem', borderRadius: '1rem', border: '1px solid #1e293b' }}>
                    <div style={{ fontSize: '0.8rem', color: '#ec4899', fontWeight: '700' }}>CIRCULAR WASTE LOOP</div>
                    <div style={{ fontSize: '1.5rem', fontWeight: '800', marginTop: '0.25rem' }}>{results.metrics.waste_kg} <span style={{ fontSize: '0.85rem', color: '#64748b' }}>kg</span></div>
                  </div>
                </div>
              </div>

              <div className="animate-fade-in interactive-card" style={{ backgroundColor: '#111c44', padding: '2rem', borderRadius: '1.5rem', border: '1px solid #1e293b' }}>
                <h4 style={{ fontSize: '1.2rem', fontWeight: '800', color: '#34d399', marginTop: 0, marginBottom: '1.25rem' }}>Mitigation Intelligence Optimization Routing</h4>
            ...
            <div className="animate-fade-in" style={{ height: '100%', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', border: '2px dashed #1e293b', borderRadius: '1.5rem', padding: '4rem 2rem', textAlign: 'center', color: '#475569', minHeight: '400px' }}>
              ...
              <p style={{ fontSize: '0.9rem', marginTop: '0.5rem', color: '#475569' }}>Provide operational vectors within the configuration matrix and compile.</p>
            </div>
          )}
        </section>
      </div>
